        ...
        if is_random_name:
            ...
            if needs_conversion:
                ...
                if needs_compression and needs_resolution_adjust:
                    ...
                    reason = "分辨率调整"
                temp_path = image_path.parent / (original_name + '_processing' + target_ext)
                if convert_to_avif(image_path, temp_path, use_gpu=use_gpu, use_ffmpeg=use_ffmpeg):
                    image_path.unlink()  # 删除原文件
                    temp_path.rename(new_path)  # 重命名临时文件
